...
Using the Linear connector, list issues on team `LOO` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `LOO-NNN-short-slug`, using the issue's real identifier.
...
- `Closes LOO-NNN`, using the real Linear issue identifier
